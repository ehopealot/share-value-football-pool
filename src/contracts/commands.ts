...
const straightLegIssues = (leg: z.infer<typeof canonicalLeg>, ctx: z.RefinementCtx) => {
  proofIssues(leg, ctx);
...
export const straightLeg = canonicalLeg.superRefine(straightLegIssues);
/** Parlays retain unadjusted complete legs, including a bookmaker proof for vig-free moneylines. */
export const parlayLeg = canonicalLeg.superRefine(straightLegIssues);
...
export const parlaySemanticIssues = (v: { legs: Array<{ eventId: string; market: "spread" | "total" | "moneyline"; selection: "home" | "away" | "over" | "under" }> }, ctx: z.RefinementCtx) => {
  try {
    validateParlay(v.legs.map((leg) => ({ ...leg, originalOdds: -110 })));
  } catch (error) {
...
export const canonicalParlayQuoteProjection = parlayWagerQuoteSnapshotBase.extend({ wagerId, actorId: z.string().min(1), fingerprint: z.string().min(1) }).strict().superRefine(parlaySnapshotIssues);
...
const commandEventPayload = z.discriminatedUnion("commandType", [
  commandEventBase.extend({ commandType: z.literal("InitializePool"), memberId: z.string().min(1) }).strict(), commandEventBase.extend({ commandType: z.literal("JoinPool"), memberId: z.string().min(1) }).strict(), commandEventBase.extend({ commandType: z.literal("UpdateMemberNickname"), memberId: z.string().min(1) }).strict(), commandEventBase.extend({ commandType: z.literal("UpdatePoolSettings") }).strict(), commandEventBase.extend({ commandType: z.literal("CreateSeason"), seasonId: z.string().min(1) }).strict(), commandEventBase.extend({ commandType: z.literal("OpenSeason"), seasonId: z.string().min(1) }).strict(), commandEventBase.extend({ commandType: z.literal("CloseSeason"), seasonId: z.string().min(1) }).strict(), commandEventBase.extend({ commandType: z.literal("ConfirmSuperBowl"), seasonId: z.string().min(1) }).strict(), commandEventBase.extend({ commandType: z.literal("ExecuteShareOrder"), seasonId: z.string().min(1), memberId: z.string().min(1), orderId: z.string().min(1) }).strict(), commandEventBase.extend({ commandType: z.literal("ReverseShareOrder"), seasonId: z.string().min(1), memberId: z.string().min(1), orderId: z.string().min(1) }).strict(), commandEventBase.extend({ commandType: z.literal("SuspendMember"), memberId: z.string().min(1) }).strict(), commandEventBase.extend({ commandType: z.literal("RestoreMember"), memberId: z.string().min(1) }).strict(), commandEventBase.extend({ commandType: z.literal("TransferCommissioner"), memberId: z.string().min(1) }).strict(), commandEventBase.extend({ commandType: z.literal("VoidWager"), wagerId: z.string().min(1) }).strict(), commandEventBase.extend({ commandType: z.literal("RegradeWager"), wagerId: z.string().min(1) }).strict(), commandEventBase.extend({ commandType: z.literal("CreateSeasonAnnotation"), seasonId: z.string().min(1) }).strict(), commandEventBase.extend({ commandType: z.literal("PlaceStraightWager"), seasonId: z.string().min(1), memberId: z.string().min(1), wagerId: z.string().min(1) }).strict(), commandEventBase.extend({ commandType: z.literal("PlaceTeaserWager"), seasonId: z.string().min(1), memberId: z.string().min(1), wagerId: z.string().min(1) }).strict(), commandEventBase.extend({ commandType: z.literal("PlaceParlayWager"), seasonId: z.string().min(1), memberId: z.string().min(1), wagerId: z.string().min(1) }).strict()
]);
