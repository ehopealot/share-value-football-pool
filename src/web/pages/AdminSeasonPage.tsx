...
export function AdminSeasonPage() {
  ...
  if (loadError) return <Layout signedIn><h1>Season administration</h1><p ref={errorRef} role="alert" tabIndex={-1} className="error-summary">{loadError} <Link to={`/p/${slug}/overview`}>Return to the pool overview</Link>.</p></Layout>;
  if (!view) return <Layout><p role="status">Loading season…</p></Layout>;
  if (view.currentMember.role !== "commissioner") return <Layout signedIn><h1>Season administration</h1><p role="alert">Only the commissioner can create or open a season.</p></Layout>;
  ...
  return <Layout signedIn><h1>Season administration</h1>{error && <p ref={errorRef} role="alert" tabIndex={-1} className="error-summary">{error}</p>}
    {view.activeSeason && <><p role="status">Active season: {view.activeSeason.label}. Funded orders and wagers are available.</p>{view.activeSeason.superBowlCandidate && <p>Canonical Super Bowl candidate: {view.activeSeason.superBowlCandidate.providerEventName}. {view.activeSeason.superBowlCandidate.confirmedAt ? "Confirmed." : <button disabled={seasonCommand.pending} onClick={() => void confirm(view.activeSeason!.id, view.activeSeason!.superBowlCandidate!.eventId)}>Confirm canonical Super Bowl</button>}</p>}</>}
    ...
    {!view.activeSeason && !draft && <form onSubmit={(e) => { e.preventDefault(); void create(); }}><label>Season label <input disabled={seasonCommand.pending} value={label} onChange={(e) => { edit(); setLabel(e.target.value); }} required /></label><fieldset disabled={seasonCommand.pending}><legend>Optional default order (form convenience only)</legend><label>Mode <select value={mode} onChange={(e) => { edit(); setMode(e.target.value as "shares" | "value"); }}><option value="shares">Shares</option><option value="value">Virtual value</option></select></label><label>Default amount <input type="number" min="0.000001" step="0.000001" value={amount} onChange={(e) => { edit(); setAmount(e.target.value); }} /></label></fieldset><button disabled={seasonCommand.pending} className="primary-action">Create season</button></form>}
    ...
    <p><Link to={`/p/${slug}/overview`}>Pool overview</Link></p></Layout>;
}
