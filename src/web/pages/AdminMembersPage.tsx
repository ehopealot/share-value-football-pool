...
export function AdminMembersPage() {
  ...
  useEffect(load, [slug]);
  if (loadError) return <Layout signedIn><h1>Member administration</h1><p role="alert" tabIndex={-1} className="error-summary">{loadError} <Link to={`/p/${slug}/overview`}>Return to the pool overview</Link>.</p></Layout>;
  if (!view) return <Layout><p role="status">Loading members…</p></Layout>;
  if (view.currentMember.role !== "commissioner") return <Layout signedIn><h1>Member administration</h1><p role="alert">Only the commissioner can manage members.</p></Layout>;
  ...
  return <Layout signedIn><h1>Member administration</h1>{error && <p role="alert" className="error-summary">{error}</p>}
    <table><caption>Active and suspended members</caption><tbody>{view.members.map((member) => {
      ...
      return <tr key={member.memberId}><th scope="row">{member.displayName}</th><td>{member.role}</td><td>{member.status}</td><td>{member.memberId !== view.currentMember.memberId && <button disabled={memberCommand.pending} onClick={() => void command(`${statusAction}:${member.memberId}`, `/admin/members/${member.memberId}/${statusAction}`, () => ({ idempotencyKey: crypto.randomUUID() }))}>{statusAction === "suspend" ? "Suspend" : "Restore"}</button>}</td><td>{member.role !== "commissioner" && member.status === "active" && <button disabled={!reason.trim() || memberCommand.pending} onClick={() => void command(`transfer:${member.memberId}`, "/admin/transfer", () => ({ memberId: member.memberId, reason, idempotencyKey: crypto.randomUUID() }))}>Transfer commissioner</button>}</td></tr>;
    })}</tbody></table>
    <label>Transfer reason <input disabled={memberCommand.pending} value={reason} onChange={(e) => { memberCommand.retire(); setError(""); setReason(e.target.value); }} /></label>
    <p>Transfers require recent authentication and an audit reason.</p><Link to={`/p/${slug}/overview`}>Pool overview</Link>
  </Layout>;
}
