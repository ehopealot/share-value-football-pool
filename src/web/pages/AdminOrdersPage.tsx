...
import { projectAdminOrders, groupOrdersByMember, type ProjectedOrder } from "./admin-orders-lifecycle";
...
type Semantic = { seasonId: string; memberId: string; mode: "shares" | "value"; amount: string; lockPriceAtOneDollar: boolean; quoteKey: string };
...
const newEditor = (view: ReadPoolView): Semantic | undefined => {
  ...
  return { seasonId: season.id, memberId: view.currentMember.memberId, mode: season.defaultOrderMode ?? "shares", amount: season.defaultOrderAmountMicros ? wholeShares(season.defaultOrderAmountMicros) : "", lockPriceAtOneDollar: false, quoteKey: crypto.randomUUID() };
};
...
export function AdminOrdersPage() {
  ...
  const quoteOrder = async () => {
    ...
    try {
      const quote = await api.quoteOrder(slug, { seasonId: request.seasonId, memberId: request.memberId, mode: request.mode, amountMicros, lockPriceAtOneDollar: request.lockPriceAtOneDollar, idempotencyKey: request.quoteKey });
  ...
  return <Layout><h1>Share orders</h1>{projection.notice && <p role="status">{projection.notice}</p>}{error && <p ref={errorRef} role="alert" tabIndex={-1} className="error-summary">{error}</p>}<div className="share-order-form"><label>Member <select aria-label="Member" disabled={pending || !projection.canOrder} value={editor?.memberId ?? ""} onChange={event => edit({ memberId: event.target.value })}>{view.members.filter(member => member.status === "active").map(member => <option key={member.memberId} value={member.memberId}>{member.displayName}</option>)}</select></label><label>Order form <select disabled={pending || !projection.canOrder} value={editor?.mode ?? "shares"} onChange={event => edit({ mode: event.target.value as "shares" | "value" })}><option value="shares">Shares</option><option value="value">Dollars</option></select></label><label>Amount <input type="number" min="1" step="1" inputMode="numeric" disabled={pending || !projection.canOrder} value={editor?.amount ?? ""} onChange={event => edit({ amount: event.target.value })} /></label><label><input type="checkbox" disabled={pending || !projection.canOrder} checked={editor?.lockPriceAtOneDollar ?? false} onChange={event => edit({ lockPriceAtOneDollar: event.target.checked })} /> Lock price at $1 per share</label><button className="primary-action" disabled={pending || !editor?.amount || !projection.canOrder} onClick={() => void quoteOrder()}>{pending ? "Quoting…" : "Quote order"}</button></div><section>{projection.seasons.length ? <><label className="order-group-toggle"><input type="checkbox" checked={!groupByMember} onChange={event => setGroupByMember(!event.target.checked)} /> Show individual orders</label>{projection.seasons.map(season => <section className="table-ribbon-section" key={season.seasonId}><h2 className="table-ribbon">Order history</h2>{season.readOnly && <p>Read-only closed-season record</p>}<div className="table-scroll" tabIndex={0}>{groupByMember ? <table><thead><tr><th>Member</th><th>Shares</th><th>Price</th><th>Total cost</th></tr></thead><tbody>{groupOrdersByMember(season.orders).map(group => <tr key={group.memberId}><td>{group.memberDisplayName}</td><td>{wholeShares(group.netSharesMicros)} shares</td><td>${display(group.blendedPriceMicros, 2)} per share</td><td>${display(group.netValueMicros, 2)}</td></tr>)}</tbody></table> : <table><thead><tr><th>Member</th><th>Shares</th><th>Price</th><th>Status</th><th>Reason</th><th>Action</th></tr></thead><tbody>{season.orders.map(order => <tr key={order.orderId}><td>{order.memberDisplayName}</td><td>{wholeShares(order.sharesMicros)} shares</td><td>${display(order.priceMicros, 2)} per share</td><td>{order.reversalStatus}</td><td>{order.reason}</td><td>{order.reversible && <button onClick={() => { setError(""); setReversal({ tag: "reviewing", order, reason: "", idempotencyKey: crypto.randomUUID() }); }}>Reverse with reason</button>}</td></tr>)}</tbody></table>}</div></section>)}</> : <p>No share orders have been issued.</p>}</section><p><Link to={`/p/${slug}/overview`}>Pool home</Link></p></Layout>;
}
