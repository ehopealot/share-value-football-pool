...
export function AdminSettingsPage() {
  ...
  return <Layout signedIn><h1>Pool settings</h1>{error && <p role="alert" className="error-summary">{error}</p>}
    ...
    <label htmlFor="commissioner-notice">Commissioner notice</label><textarea id="commissioner-notice" className="commissioner-notice-input" disabled={settings.pending} value={commissionerNotice} maxLength={500} onChange={(e) => { edit(); setCommissionerNotice(e.target.value); }} /><button disabled={!commissionerNotice.trim() || settings.pending} onClick={() => void save(`notice:${commissionerNotice}`, () => ({ commissionerNotice }))}>Save notice</button>{view.pool.commissionerNotice !== null && <button disabled={settings.pending} onClick={() => void save("clear-notice", () => ({ commissionerNotice: null }))}>Clear notice</button>}
