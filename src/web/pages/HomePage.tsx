...
export function HomePage() {
  ...
  if (!user) return <Layout><h1>Private football pool</h1><ErrorSummary message={error}/><p>Run a private season with virtual shares and clear, locked terms. No payments or cash value are involved.</p><p><Link className="primary-action" to="/sign-up">Create account</Link> <Link to="/login">Log in</Link></p></Layout>;
  if (memberships === undefined && !error) return <Layout signedIn><h1>Your pools</h1><p role="status">Loading pool memberships…</p></Layout>;
  if (error) return <Layout signedIn><h1>Your pools</h1><ErrorSummary message={error}/><StateNotice title="Pool list unavailable"><p>Your account is still signed in. Return to a pool link or try Home again.</p><button type="button" onClick={retry}>Reload Home</button></StateNotice></Layout>;
  return <Layout signedIn><h1>Your pools</h1>{memberships!.length ? <table><caption>Your active memberships</caption><thead><tr><th scope="col">Pool</th><th scope="col">Role</th><th scope="col">Next action</th></tr></thead><tbody>{memberships!.map((pool) => <tr key={pool.poolId}><td>{pool.poolName}</td><td>{pool.role}</td><td><Link to={`/p/${pool.slug}`}>Enter pool</Link></td></tr>)}</tbody></table> : <StateNotice title="No pools yet"><p>Create a pool or use a private pool link from its commissioner.</p><Link className="primary-action" to="/pools/new">Create a pool</Link></StateNotice>}</Layout>;
}
