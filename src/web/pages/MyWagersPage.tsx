...
export function MyWagersPage() {
  ...
  const weeks = wagerWeekOptions(data.wagers.map((wager) => wager.weekStart), currentWeek);
  const week = selectedWeekOrCurrent(selectedWeek, weeks, currentWeek);
  const selectedWagers = week === undefined ? data.wagers : data.wagers.filter((wager) => wager.weekStart === week);
