...
export type ParlayViewState = { tag: "editing"; editor: ParlaySemantic } | { tag: "quoting"; request: ParlaySemantic } | { tag: "reviewing"; request: ParlaySemantic; quote: any; mutationKey: string } | { tag: "submitting"; request: ParlaySemantic; quote: any; mutationKey: string };
...
export function ParlayPage() {
  const { slug = "" } = useParams(); const nav = useNavigate(); const [view, setView] = useState<any>(); const [state, setState] = useState<State>(() => ({ tag: "editing", editor: fresh([]) })); const [error, setError] = useState(""); const errorRef = useRef<HTMLParagraphElement>(null);
  useEffect(() => { const legs = readParlaySlip(slug); setState({ tag: "editing", editor: fresh(legs) }); void api.poolView(slug).then(setView).catch((e) => setError(errorMessage(e))); }, [slug]);
  ...
  const recover = async (request: ParlaySemantic) => {
  ...
  const place = async () => {
    if (!reviewed || reviewed.tag !== "reviewing") return;
    setState({ ...reviewed, tag: "submitting" });
    try {
      await api.placeCommand(slug, "/wagers/parlays/place", buildParlayPlacement(reviewed.quote, reviewed.request.wagerId, reviewed.mutationKey));
      writeParlaySlip(slug, []);
      nav(`/p/${slug}/my-wagers`);
    } catch (e) {
      if (commandOutcome(e) === "stale") {
        try { await recover(reviewed.request); }
        catch { setState(reviewed); setError("Odds unavailable."); }
      } else if (commandOutcome(e) === "terminal") setState(parlayTerminalTransition(reviewed.request)); else setState(reviewed);
      if (commandOutcome(e) !== "stale") setError(errorMessage(e));
    }
  };
  if (reviewed) return <Layout signedIn><Confirmation snapshot={{ kind: "parlay", quote: reviewed.quote }} /><div className="confirmation-actions"><button className="primary-action" disabled={pending} onClick={() => void place()}>{pending ? "Confirming…" : "Place parlay"}</button><button disabled={pending} onClick={() => returnToEditor(reviewed.request)}>Edit terms</button></div>{error && <p ref={errorRef} role="alert" tabIndex={-1} className="error-summary">{error}</p>}</Layout>;
