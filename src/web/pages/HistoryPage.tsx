...
export function HistoryPage() {
  ...
  if (loadError) return <Layout signedIn><h1>Archived season</h1><p ref={errorRef} tabIndex={-1} role="alert" className="error-summary">{loadError} <Link to={`/p/${slug}/overview`}>Return to the pool overview</Link>.</p></Layout>;
  ...
  return <Layout signedIn><h1>Archived season: {data.season.label}</h1><p>Read-only {data.season.state} season. {data.season.closeReason ? `Closed: ${data.season.closeReason}.` : ""}</p>
    ...
    {error && <p ref={errorRef} tabIndex={-1} role="alert" className="error-summary">{error}</p>}<p><Link to={`/p/${slug}/overview`}>Pool overview</Link></p>
  </Layout>;
}
