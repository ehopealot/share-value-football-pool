...
function WagerRows({ wager, dayAnchorStartsAt }: { wager: Wager; dayAnchorStartsAt?: string }) {
  const legs = sortWagerLegsByStartTime(wager.legs ?? []);
...
export function DayActivitySection({ day, memberProfilePath }: { day: ReturnType<typeof groupActivityDaysForWeek>[number]; memberProfilePath: (memberId: string) => string }) {
  return <section className="activity-day-section"><h3 className="activity-day-ribbon">{day.label}</h3><div className="table-scroll" tabIndex={0}><table className="activity-table"><colgroup><col className="activity-start-column"/><col className="activity-wager-column"/><col className="activity-staked-column"/><col className="activity-pnl-column"/></colgroup><thead><tr><th>Start</th><th>Wager</th><th>Staked</th><th>P&amp;L</th></tr></thead>{day.members.map((member) => <tbody key={member.memberId}><tr className="activity-day-member-ribbon"><th colSpan={4} scope="rowgroup"><span><Link to={memberProfilePath(member.memberId)}>{member.memberDisplayName}</Link><small>{formatActivityPerformance(member.performanceMicros)}</small></span></th></tr>{sortWagersByStartTime(member.wagers).flatMap((wager) => [...(day.upcoming ? [<tr className="wager-date-row" key={`${wager.wagerId}:date`}><th colSpan={4}>{displayWagerDateLabel(wager)}</th></tr>] : []), <WagerRows key={wager.wagerId} wager={wager} dayAnchorStartsAt={day.startsAt}/>])}</tbody>)}</table></div></section>;
}
