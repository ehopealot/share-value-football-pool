...
export function PoolGatePage() {
  ...
  useEffect(() => { void api.gate(slug).then((result) => { setSignedIn(true); if (result.membership === "member") nav(`/p/${slug}/overview`, { replace: true }); else setGate(result); }).catch((e: ApiError) => { if (e.status === 401) nav(`/login?next=${encodeURIComponent(`/p/${slug}`)}`, { replace: true }); else setError("This private pool is unavailable. Return home and try again."); }); }, [nav, slug]);
  const join = async (e: FormEvent<HTMLFormElement>) => { e.preventDefault(); setPending(true); setError(""); try { await api.joinPool(slug, String(new FormData(e.currentTarget).get("password")), commandId.current, { turnstileToken: await acquireTurnstileToken(turnstileTarget.current) }); commandId.current = crypto.randomUUID(); nav(`/p/${slug}/overview`); } catch (error) { setError(joinErrorMessage(error)); } finally { setPending(false); } };
  if (!gate && !error) return <Layout signedIn={signedIn}><p role="status">Loading pool entry…</p></Layout>;
  if (gate?.membership === "closed") return <Layout signedIn={signedIn}><StateNotice title="This pool is not accepting members"><p>Ask the commissioner if you need access. No pool information is available here.</p><Link to="/">Return home</Link></StateNotice></Layout>;
