...
export function PoolGatePage() {
  const { slug = "" } = useParams(); const nav = useNavigate(); const turnstileTarget = useRef<HTMLDivElement>(null); const [gate, setGate] = useState<PoolGate>(); const [error, setError] = useState(""); const [pending, setPending] = useState(false); const commandId = useRef(crypto.randomUUID());
  useEffect(() => { void api.gate(slug).then((result) => { if (result.membership === "member") nav(`/p/${slug}/odds`, { replace: true }); else setGate(result); }).catch((e: ApiError) => { if (e.status === 401) nav(`/login?next=${encodeURIComponent(`/p/${slug}`)}`, { replace: true }); else setError("This private pool is unavailable. Return home and try again."); }); }, [nav, slug]);
