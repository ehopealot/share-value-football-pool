...
export const parseShareOrderQuoteSuccess = (request: OrderQuoteRequest, value: unknown, expectedMemberId = request.memberId) => {
  ...
  if (quote.seasonId !== request.seasonId || quote.memberId !== request.memberId || quote.memberId !== expectedMemberId || quote.mode !== request.mode || quote.amountMicros !== request.amountMicros || quote.lockPriceAtOneDollar !== request.lockPriceAtOneDollar) responseMismatch();
  return quote;
};
...
export const buildShareOrderExecution = (quote: z.infer<typeof shareOrderQuoteSnapshot>, mutationKey: string, reason: string) => executeShareOrderRequest.parse({ seasonId: quote.seasonId, memberId: quote.memberId, mode: quote.mode, amountMicros: quote.amountMicros, lockPriceAtOneDollar: quote.lockPriceAtOneDollar, quote: { priceMicros: quote.priceMicros, commandVersion: quote.commandVersion }, reason, idempotencyKey: mutationKey });
