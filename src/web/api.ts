...
export const errorMessage = (error: unknown) => {
  if (!(error instanceof ApiError)) return "The service did not respond. Your command was not changed; retry with the same terms.";
  const messages: Record<string, string> = { LINE_CHANGED: "The line changed. Review the replacement terms and confirm again.", SUSPENDED: "Your access to this pool is suspended. Ask the commissioner to restore it.", INSUFFICIENT_SHARES: "You do not have enough available shares for this risk.", WHOLE_SHARE_RISK_REQUIRED: "Wager risk must be a whole number of shares.", MARKET_STALE: "This offer is stale. Wait for a current feed.", MARKET_UNAVAILABLE: "This market is unavailable.", MARKET_LOCKED: "This event is locked because it has started.", WAGER_NOT_STARTED: "Regrade is available only after every leg starts. Before then, Void remains available.", SEASON_CLOSED: "This season is closed. You can review its history but cannot place new wagers.", SEASON_NOT_ACTIVE: "There is no active season for this command.", SEASON_NOT_CLOSED: "This season is still active or in draft. Open its archive after the season closes.", ORDER_QUOTE_STALE: "The share price changed. Request and confirm the replacement order quote.", ORDER_REVERSAL_INSUFFICIENT_AVAILABLE_SHARES: "This order cannot be reversed because the member no longer has its available shares.", RECENT_AUTH_REQUIRED: "For this protected change, sign in again (or reauthenticate) and then retry the same confirmation.", IDEMPOTENCY_CONFLICT: "This command key belongs to different terms. Review and submit the current terms.", POOL_NOT_AVAILABLE: "The pool is temporarily unavailable. Retry this same confirmation.", POOL_UNAVAILABLE: "The pool is temporarily unavailable. Retry this same confirmation." };
  return messages[error.code] ?? `Unable to complete this request (${error.code}).`;
};
