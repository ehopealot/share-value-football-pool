...
export function shapeWagers(sql: SqlStorage, viewerId: string, now = new Date(), ownerOnly = false, seasonId?: string, exposeStartedRisk = false): { wagers: unknown[] } {
  return shapeWagersWithPolicy(sql, viewerId, now, ownerOnly, seasonId, "owner-or-started", exposeStartedRisk);
}
...
function shapeWagersWithPolicy(sql: SqlStorage, viewerId: string, now: Date, ownerOnly: boolean, seasonId: string | undefined, legRevealPolicy: LegRevealPolicy, exposeStartedRisk = false): { wagers: unknown[] } {
  ...
  const wagers = [...sql.exec<Row>(query, ...(ownerOnly ? [viewerId] : []), ...(seasonId ? [seasonId] : []))].map((wager) => {
    ...
    const riskIsVisible = ownsTicket || (exposeStartedRisk && (revealed.length > 0 || settlement !== undefined));
    return redactRecursively({ wagerId: String(wager.id), seasonId: String(wager.season_id), memberId: String(wager.owner_id), memberDisplayName: String(wager.owner_display_name), type: String(wager.type), status: String(wager.status), confirmedAt: String(wager.confirmed_at), weekStart: weekStartOf(new Date(String(wager.activity_week_start))).toISOString(), performanceMicros, ...(riskIsVisible ? { riskMicros: String(wager.risk_micros) } : {}), ...(ownsTicket ? { acceptedOdds: Number(wager.accepted_odds), rulesetVersion: String(wager.ruleset_version), ...(settlement ?? {}) } : {}), ...(revealed.length ? { legs: revealed } : {}) });
  });
  return { wagers };
}
