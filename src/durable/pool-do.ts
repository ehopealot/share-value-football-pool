...
/** Standalone commissioner notices and rules are informational and must not affect wagering or projections. */
const isInformationalSettingsCommand = (command: PoolCommand) => command.type === "UpdatePoolSettings" && command.poolName === undefined && command.password === undefined && command.signupsOpen === undefined && command.maxSideBetMicros === undefined && (command.commissionerNotice !== undefined || command.commissionerRules !== undefined);
/** Board conversation and standalone commissioner notices and rules are authoritative-only, without D1 projection or alarm work. */
const shouldEnqueueOutbox = (command: PoolCommand) => !isReadCommand(command) && !isQuoteCommand(command) && command.type !== "CreateMessageBoardPost" && command.type !== "ReplyToMessageBoardPost" && !isInformationalSettingsCommand(command);
...
export class PoolDO {
  ...
  private authorized(sql: SqlStorage, command: Exclude<PoolCommand, { type: "InitializePool" }>): PoolCommandResult {
    const pool = first(sql, "SELECT commissioner_id, password_hash, signups_open, max_side_bet_micros, commissioner_notice, commissioner_rules, command_version, active_season_id FROM pool LIMIT 1");
    ...
    if (command.type === "UpdatePoolSettings") {
      ...
      sql.exec("UPDATE pool SET name = ?, password_hash = ?, password_version = password_version + ?, signups_open = ?, max_side_bet_micros = ?, commissioner_notice = ?, commissioner_rules = ?", command.poolName?.trim() || first(sql, "SELECT name FROM pool LIMIT 1")!.name, passwordHash, command.password === undefined ? 0 : 1, command.signupsOpen === undefined ? first(sql, "SELECT signups_open FROM pool LIMIT 1")!.signups_open : command.signupsOpen ? 1 : 0, command.maxSideBetMicros ?? String(pool.max_side_bet_micros), command.commissionerNotice === undefined ? pool.commissioner_notice ?? null : command.commissionerNotice, command.commissionerRules === undefined ? pool.commissioner_rules ?? null : command.commissionerRules);
      return { commandVersion: isInformationalSettingsCommand(command) ? String(pool.command_version) : this.bumpVersion(sql) };
    }
  ...
  private readPool(sql: SqlStorage, actorId: string, commandVersion: string, activeSeasonId: SqlStorageValue | undefined): PoolCommandResult {
    const pool = first(sql, "SELECT id, slug, name, commissioner_id, signups_open, max_side_bet_micros, commissioner_notice, commissioner_rules FROM pool LIMIT 1")!;
