  ...
  if (due.length) try {
    ...
    await state.storage.transaction(async () => {
      ...
      for (const lifecycle of due) {
        ...
        if (String(lifecycle.phase) === "open") state.storage.sql.exec("UPDATE event_reconciliation SET phase = 'final_15', attempts = 0, error_attempts = 0, final_observed_at = ?, deadline_at = ?, next_attempt_at = ?, last_error = NULL WHERE event_id = ?", at(observed), at(observed + FINAL_15_MINUTES), at(observed + FINAL_15_MINUTES), lifecycle.event_id);
        else if (String(lifecycle.phase) === "final_15") state.storage.sql.exec("UPDATE event_reconciliation SET phase = 'final_24', attempts = 0, error_attempts = 0, deadline_at = ?, next_attempt_at = ?, last_error = NULL WHERE event_id = ?", at(observed + FINAL_24_HOURS), at(observed + FINAL_24_HOURS), lifecycle.event_id);
