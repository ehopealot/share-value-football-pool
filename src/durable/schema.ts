...
export const migrateAdditivePoolStorage = (sql: SqlStorage): void => {
  ...
  if (!poolColumns.some((column) => column.name === "commissioner_rules")) sql.exec("ALTER TABLE pool ADD COLUMN commissioner_rules TEXT CHECK(commissioner_rules IS NULL OR length(trim(commissioner_rules)) BETWEEN 1 AND 4000)");
