...
export const poolSchema = [
  ...
  `CREATE INDEX IF NOT EXISTS message_board_thread_activity_idx ON message_board_entry(parent_post_id, activity_at DESC, created_at ASC, id ASC)`,
  `CREATE INDEX IF NOT EXISTS message_board_reply_order_idx ON message_board_entry(parent_post_id, created_at ASC, id ASC)`,
...
export const migrateAdditivePoolStorage = (sql: SqlStorage): void => {
  ...
  const wagerLegColumns = [...sql.exec<{ name: string }>("PRAGMA table_info(wager_leg)")];
  if (!wagerLegColumns.some((column) => column.name === "home_score")) sql.exec("ALTER TABLE wager_leg ADD COLUMN home_score INTEGER");
  if (!wagerLegColumns.some((column) => column.name === "away_score")) sql.exec("ALTER TABLE wager_leg ADD COLUMN away_score INTEGER");
