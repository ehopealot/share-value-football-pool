...
export type OrderInput = {
  ...
  mode: "shares" | "value"; amountMicros: string; lockPriceAtOneDollar: boolean; quote: { priceMicros: string; commandVersion: string }; reason: string;
};

export function quoteShareOrder(sql: Sql, seasonId: string, memberId: string, mode: "shares" | "value", amountMicros: string, lockPriceAtOneDollar = false) {
  const quote = new AccountingRepository(sql).quote(seasonId);
  const requested = parseIntegerText(amountMicros);
  const priceMicros = lockPriceAtOneDollar ? MICROS_PER_UNIT : quote.priceMicros;
  const { sharesMicros: shares, valueMicros: value } = calculateShareOrderAmounts(mode, requested, priceMicros);
  if (shares <= 0n || value <= 0n) throw new Error("ORDER_ROUNDS_BELOW_ONE_MICRO");
  return { seasonId, memberId, mode, amountMicros, lockPriceAtOneDollar, priceMicros: priceMicros.toString(), currentPriceMicros: quote.priceMicros.toString(), commandVersion: quote.commandVersion, sharesMicros: shares.toString(), valueMicros: value.toString() };
}
...
export function executeShareOrder(sql: Sql, input: OrderInput, now = new Date().toISOString()) {
  const result = new AccountingRepository(sql).applyOrder({
    ...
    priceMicros: parseIntegerText(input.quote.priceMicros), lockPriceAtOneDollar: input.lockPriceAtOneDollar, commandVersion: input.quote.commandVersion,
