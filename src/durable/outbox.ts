...
export async function drainOutbox(state: DurableObjectState, queue?: Queue<PoolOutboxMessage>, now = new Date(), db?: D1Database): Promise<{ pending: boolean }> {
  ...
  for (const row of rows) {
    ...
    if (!parsed.success) { state.storage.sql.exec(`UPDATE outbox SET attempts = ${MAX_OUTBOX_ATTEMPTS}, last_error = 'INVALID_OUTBOX_EVENT' WHERE id = ?`, row.id); continue; }
...
export function nextOutboxAttempt(state: DurableObjectState): number | null {
  const row = [...state.storage.sql.exec<Row>(`SELECT next_attempt_at FROM outbox WHERE delivered_at IS NULL AND attempts < ${MAX_OUTBOX_ATTEMPTS} ORDER BY next_attempt_at LIMIT 1`)][0];
