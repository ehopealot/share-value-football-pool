...
const poolCommandSchemaBase = z.discriminatedUnion("type", [
  ...
  z.object({ type: z.literal("UpdatePoolSettings"), ...common, poolName: z.string().trim().min(1).optional(), password: z.string().min(8).optional(), signupsOpen: z.boolean().optional(), maxSideBetMicros: positiveIntegerText.optional(), commissionerNotice: z.string().trim().min(1).max(500).nullable().optional(), commissionerRules: z.string().trim().min(1).max(20000).nullable().optional() }).strict(),
  ...
  z.object({ type: z.literal("QuoteShareOrder"), ...common, seasonId: z.string().min(1), memberId: z.string().min(1), mode: z.enum(["shares", "value"]), amountMicros: positiveIntegerText, lockPriceAtOneDollar: z.boolean().default(false) }),
  z.object({ type: z.literal("ExecuteShareOrder"), ...common, seasonId: z.string().min(1), memberId: z.string().min(1), mode: z.enum(["shares", "value"]), amountMicros: positiveIntegerText, lockPriceAtOneDollar: z.boolean().default(false), quote: z.object({ priceMicros: integerText, commandVersion: integerText }), reason: z.string().trim().min(1).max(500) }),
  ...
  z.object({ type: z.literal("CreateMessageBoardPost"), ...common, text: z.string().trim().min(1).max(1000), announcement: z.boolean().default(false) }).strict(),
