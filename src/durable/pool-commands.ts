...
const poolCommandSchemaBase = z.discriminatedUnion("type", [
  ...
  z.object({ type: z.literal("UpdatePoolSettings"), ...common, poolName: z.string().trim().min(1).optional(), password: z.string().min(8).optional(), signupsOpen: z.boolean().optional(), maxSideBetMicros: positiveIntegerText.optional(), commissionerNotice: z.string().trim().min(1).max(500).nullable().optional(), commissionerRules: z.string().trim().min(1).max(4000).nullable().optional() }).strict(),
