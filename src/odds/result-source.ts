...
export class D1ResultSource implements ResultSource {
  ...
  async getFinalResults(eventIds: readonly string[]): Promise<FinalResultVersion[]> {
    ...
    const result = await this.db.prepare(`SELECT provider_event_id, league, status, home_score, away_score, correction_version, event_name, postseason FROM sports_event WHERE provider_event_id IN (${placeholders}) AND status IN ('final','cancelled','no_contest')`).bind(...eventIds).all<Record<string, string | null>>();
