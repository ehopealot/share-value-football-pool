...
export function memberAuditExport(sql: SqlStorage, memberId: string, currentTime = new Date()): Record<string, unknown> {
  ...
  return {
    ...
    settlements: rows(sql, "SELECT id, wager_id, result_version, outcome, return_micros, profit_micros, settled_odds, source_result_json, reversal_of, actor_id, reason, created_at FROM settlement ORDER BY created_at, rowid").map((row) => ({ id: String(row.id), wagerId: String(row.wager_id), resultVersion: String(row.result_version), outcome: String(row.outcome), returnMicros: String(row.return_micros), profitMicros: String(row.profit_micros), settledOdds: row.settled_odds === null ? null : Number(row.settled_odds), sourceResult: JSON.parse(String(row.source_result_json)), reversalOf: text(row.reversal_of), actorId: String(row.actor_id), reason: text(row.reason), createdAt: String(row.created_at) })),
