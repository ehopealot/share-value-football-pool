...
### Serial-gate lock recovery

`npm run t10:serial-gate` uses a checkout-specific lock under `/tmp` and removes it on normal completion. An existing lock—including an empty, malformed, or stale one—is never reclaimed automatically. If acquisition is refused, use the exact lock path in the error and inspect its `owner.json` for the owner PID and active-stage process group. Stop new gate launches and confirm **all** previous gate owners and stage groups have exited before manually removing that lock directory and retrying. Missing metadata is not proof that no stage is running.
