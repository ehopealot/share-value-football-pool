...
import { formatActivityLeg, formatActivityWagerPerformance, groupActivityMembersForWeek, formatWeeklyPerformance } from "../src/web/activity-presentation";
...
describe("activity presentation", () => {
  ...
  it("omits zero P&L only when the wager amount has been redacted", () => {
    expect(formatActivityWagerPerformance(wager({ riskMicros: undefined, performanceMicros: "0" }))).toBe("");
    expect(formatActivityWagerPerformance(wager({ riskMicros: "1000000", performanceMicros: "0" }))).toBe("0.00 shares");
