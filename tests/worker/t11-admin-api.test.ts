...
const app = (user: { id: string; name: string } | null, recentlyAuthenticated?: () => Promise<boolean>, poolNotifier?: { notifyPoolJoin(message: unknown): Promise<void>; notifyCommissionerTransfer(message: unknown): Promise<void>; notifyShareOrderFulfilled(message: unknown): Promise<void>; notifyCommissionerAnnouncement(message: unknown): Promise<void> }) => createWorkerApp({ db: bindings.DB, pools: bindings.POOL_DO, commandAuthenticatorKey: bindings.POOL_COMMAND_AUTHENTICATOR_KEY, currentUser: async () => user, ...(recentlyAuthenticated ? { recentlyAuthenticated: async () => recentlyAuthenticated() } : {}), ...(poolNotifier ? { poolNotifier } : {}) });
...
describe("T11 administration HTTP commands and prohibitions", () => {
  it("transfers commissioner authority only with recent auth, a reason, and an active target", async () => {
    ...
    const notifier = { async notifyPoolJoin(_message: unknown) {}, async notifyCommissionerTransfer(message: unknown) { notifications.push(message); }, async notifyShareOrderFulfilled(_message: unknown) {}, async notifyCommissionerAnnouncement(_message: unknown) {} };
  ...
  it("settings rename and signup toggles skip recent auth while password rotation demands it", async () => {
    ...
    expect(((await (await app({ id: "member", name: "Member" }).fetch(request(`/api/p/${slug}/view`, undefined, "GET"))).json()) as any).pool.commissionerRules).toBeNull();
