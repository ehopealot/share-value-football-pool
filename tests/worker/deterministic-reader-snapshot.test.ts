...
/** Delegates every query to real D1; it only places a poll commit around the targeted reader operation. */
function readerBarrier(real: D1Database, boundary: Boundary, commit: () => Promise<void>) {
    ...
    const proxy = new Proxy(statement as object, {
      get(_value, property) {
        ...
        return async (...args: unknown[]) => {
          ...
          if (fired || !target(sql)) return invoke();
          fired = true;
  ...
  const db = new Proxy(real as object, {
    get(_value, property) {
      ...
      if (property === "batch") return async (statements: D1PreparedStatement[]) => {
        ...
        if (!fired && sql.some(target)) {
          fired = true; targetBatchStatements = sql.filter(target).length;
          if (boundary === "before") await commit();
          const result = await real.batch(statements.map((statement) => wrappedToReal.get(statement as object) ?? statement));
          if (boundary === "after") await commit();
          return result;
        }
        return real.batch(statements.map((statement) => wrappedToReal.get(statement as object) ?? statement));
      };
  ...
  return { db, proof: () => ({ fired, targetBatchStatements }) };
}
...
describe("deterministic D1 reader snapshots", () => {
  it.each(["after", "before"] as const)("returns an authenticated board from the complete %s-poll snapshot", async (boundary) => {
    ...
    expect(barrier.proof()).toEqual({ fired: true, targetBatchStatements: 2 });
  ...
  it.each([["same-event", ["teaser-same", "teaser-same"], ["spread", "total"]], ["different-event", ["teaser-one", "teaser-two"], ["spread", "spread"]]] as const)("quotes %s teaser legs without crossing the first-leg boundary", async (_name, ids, markets) => {
    for (const boundary of ["after", "before"] as const) {
      ...
      expect(response.status).toBe(200); const quote = await response.json() as any; const state = expected(boundary);
      expect(barrier.proof()).toEqual({ fired: true, targetBatchStatements: 3 });
  ...
  it.each(["after", "before"] as const)("revalidates placement from one complete %s-poll snapshot and rejects new-state turnover without mutation", async (boundary) => {
