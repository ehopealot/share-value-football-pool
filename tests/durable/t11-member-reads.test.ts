...
describe("T11 authoritative member reads", () => {
  ...
  it("publishes completed parlay stakes and accepted odds while keeping settlement odds owner-only", async () => {
    ...
    await storage(slug, (state) => state.storage.sql.exec(`
      ...
      INSERT INTO wager_leg (id, wager_id, event_id, league, canonical_book, retrieved_at, policy_version, offer_version, canonical_offer_id, canonical_proof_json, market, selection, original_line, original_odds, teaser_adjustment, adjusted_line, event_starts_at, is_super_bowl, grade, result_version, home_score, away_score) VALUES ('parlay-leg', 'parlay', 'future-parlay-event', 'nfl', 'DraftKings', '2026-01-01T00:00:00.000Z', 'CANONICAL_BOOKS_2026_V1', 'v1', NULL, NULL, 'spread', 'home', '-3', 100, NULL, NULL, '2099-01-01T00:00:00.000Z', 0, 'win', 'final-1', 0, 17);
    ...
    expect(owner.activity.wagers[0]).toMatchObject({ type: "parlay", acceptedOdds: 300, settledOdds: 250, outcome: "won", returnMicros: "3500000", legs: [expect.objectContaining({ grade: "win", homeScore: 0, awayScore: 17 })] });
  ...
  it("omits invalid persisted score pairs while retaining complete safe scores", async () => {
    const slug = `t11-invalid-scores-${crypto.randomUUID()}`;
    await initialize(slug, "Owner");
    await join(slug, "m", "Member");
    await draftSeason(slug, "s1", "2026");
    await storage(slug, (state) => state.storage.sql.exec(`
      INSERT INTO wager (id, season_id, owner_id, type, risk_micros, accepted_odds, status, ruleset_version, confirmed_at) VALUES ('score-guard', 's1', 'm', 'parlay', '1000000', 300, 'open', 'PARLAY_2026_V1', '2026-01-01T00:00:00.000Z');
      INSERT INTO wager_leg (id, wager_id, event_id, league, canonical_book, retrieved_at, policy_version, offer_version, canonical_offer_id, canonical_proof_json, market, selection, original_line, original_odds, teaser_adjustment, adjusted_line, event_starts_at, is_super_bowl, grade, result_version, home_score, away_score) VALUES
        ('safe-zero', 'score-guard', 'safe-zero-event', 'nfl', 'DraftKings', '2026-01-01T00:00:00.000Z', 'CANONICAL_BOOKS_2026_V1', 'v1', NULL, NULL, 'spread', 'home', '-3', 100, NULL, NULL, '2099-01-01T00:00:00.000Z', 0, 'win', 'score-v1', 0, 17),
        ('safe-ordinary', 'score-guard', 'safe-ordinary-event', 'nfl', 'DraftKings', '2026-01-01T00:00:00.000Z', 'CANONICAL_BOOKS_2026_V1', 'v1', NULL, NULL, 'spread', 'home', '-3', 100, NULL, NULL, '2099-01-01T00:00:00.000Z', 0, 'win', 'score-v1', 24, 17),
        ('unsafe-score', 'score-guard', 'unsafe-score-event', 'nfl', 'DraftKings', '2026-01-01T00:00:00.000Z', 'CANONICAL_BOOKS_2026_V1', 'v1', NULL, NULL, 'spread', 'home', '-3', 100, NULL, NULL, '2099-01-01T00:00:00.000Z', 0, 'win', 'score-v1', 9007199254740992, 17),
        ('negative-score', 'score-guard', 'negative-score-event', 'nfl', 'DraftKings', '2026-01-01T00:00:00.000Z', 'CANONICAL_BOOKS_2026_V1', 'v1', NULL, NULL, 'spread', 'home', '-3', 100, NULL, NULL, '2099-01-01T00:00:00.000Z', 0, 'win', 'score-v1', -1, 17),
        ('partial-score', 'score-guard', 'partial-score-event', 'nfl', 'DraftKings', '2026-01-01T00:00:00.000Z', 'CANONICAL_BOOKS_2026_V1', 'v1', NULL, NULL, 'spread', 'home', '-3', 100, NULL, NULL, '2099-01-01T00:00:00.000Z', 0, 'win', 'score-v1', NULL, 17);
      INSERT INTO wager_leg_snapshot (wager_leg_id, home_team, away_team) VALUES
        ('safe-zero', 'Home', 'Away'), ('safe-ordinary', 'Home', 'Away'), ('unsafe-score', 'Home', 'Away'), ('negative-score', 'Home', 'Away'), ('partial-score', 'Home', 'Away');
    `));
    const mine = await send(slug, { type: "ReadMyWagers", commandId: "read-invalid-scores", actorId: "m" });
    const legs = mine.wagers.find((wager: any) => wager.wagerId === "score-guard").legs;
    expect(legs).toEqual(expect.arrayContaining([
      expect.objectContaining({ eventId: "safe-zero-event", homeScore: 0, awayScore: 17 }),
      expect.objectContaining({ eventId: "safe-ordinary-event", homeScore: 24, awayScore: 17 })
    ]));
    for (const eventId of ["unsafe-score-event", "negative-score-event", "partial-score-event"]) {
      const leg = legs.find((item: any) => item.eventId === eventId);
      expect(leg).not.toHaveProperty("homeScore");
      expect(leg).not.toHaveProperty("awayScore");
    }
  }, 90_000);
