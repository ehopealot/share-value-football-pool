...
const fund = async (slug: string, commandId: string, memberId: string, seasonId = "s1") => {
  const quote = await send(slug, { type: "QuoteShareOrder", commandId: `${commandId}-quote`, actorId: "owner", seasonId, memberId, mode: "shares", amountMicros: "2000000", lockPriceAtOneDollar: false });
  return send(slug, { type: "ExecuteShareOrder", commandId, actorId: "owner", seasonId, memberId, mode: "shares", amountMicros: "2000000", lockPriceAtOneDollar: false, quote: { priceMicros: String(quote.priceMicros), commandVersion: String(quote.commandVersion) }, reason: `funding ${memberId}` });
};
