...
describe("immutable confirmation protocol", () => {
  ...
  it("renders straight, teaser, and order confirmations from snapshots without editor data", () => {
    ...
    expect(teaser).toContain("Snapshot Away at Snapshot Home"); expect(teaser).toContain("6-point teaser"); expect(teaser).toContain("possible profit 0.83 shares; total return 1.83 shares."); expect(order).toContain("Issue 1.00 shares worth 1.00 virtual value to Frozen Member"); expect(order).toContain("Locked price: 1.0000 per share (1.000000 exact six-decimal terms); command version 1.");
