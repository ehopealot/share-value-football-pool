...
const waitFor = async (path: string, timeoutMs = OWNER_TIMEOUT_MS) => {
  ...
  while (true) {
    ...
    if (Date.now() >= expiresAt) throw new Error(`wait for ${path} timed out after ${timeoutMs}ms`);
    await delay(Math.min(10, Math.max(0, expiresAt - Date.now())));
  }
};
type ControlRecord = { state: string; pid: number; pgid: number; persistence: string };
const controlRecord = async (directory: string, id: string, state: string, timeoutMs = OWNER_TIMEOUT_MS) => {
  const path = state === "RESOURCE_CREATED" ? join(directory, `${id}.resource.json`) : join(directory, `${id}.json`);
  const expiresAt = Date.now() + timeoutMs;
  while (true) {
    try {
      const record = JSON.parse(await readFile(path, "utf8")) as ControlRecord;
      if (record.state === state) return record;
    } catch (error) {
      if (!(error instanceof SyntaxError) && (error as NodeJS.ErrnoException).code !== "ENOENT") throw error;
    }
    if (Date.now() >= expiresAt) throw new Error(`${id} ${state} timed out after ${timeoutMs}ms`);
    await delay(Math.min(10, Math.max(0, expiresAt - Date.now())));
  }
};
...
const absent = async (path: string, label: string, timeoutMs = OWNER_TIMEOUT_MS) => {
  const expiresAt = Date.now() + timeoutMs;
  while (true) {
    try { await access(path); }
    catch (error) { if ((error as NodeJS.ErrnoException).code === "ENOENT") return; throw error; }
    if (Date.now() >= expiresAt) throw new Error(`${label} timed out after ${timeoutMs}ms`);
    await delay(Math.min(10, Math.max(0, expiresAt - Date.now())));
  }
};
...
describe("owned local process protocol", () => {
  ...
  it("owns prerequisite builds and all real owner cleanup and pre-ready failures within deadlines", async () => {
    ...
    const fallbackOwners = new Set<ChildProcess>();
    const fallbackRecords = new Map<string, { record: ControlRecord; childGroupAbsent: boolean }>();
    const heldOwners: Array<{ id: string; owner: ChildProcess }> = [];
    const cleanupErrors: unknown[] = [];
    const preReadyFallbackActions: string[] = [];
    const outerReportedChildSignals: string[] = [];
    let injectedFallbackFailure = false;
    let injectedPersistenceRemovalFailure = false;
    let persistenceRemovalFailureId: string | undefined;
    const cleanupFallback = async (id: string, owner: ChildProcess | undefined, record: ControlRecord | undefined, release: boolean, trackPreReadyActions = false) => {
      const fallbackRecord = record ? fallbackRecords.get(id) ?? { record, childGroupAbsent: false } : undefined;
      if (fallbackRecord) fallbackRecords.set(id, fallbackRecord);
      ...
      let ownerGroupAbsent = !owner?.pid;
      await attempt("owner group absence", async () => {
        if (owner?.pid && !await waitForProcessGroupExit(owner.pid, 30, 50, { cleanupTimeoutMs: 10_000 })) throw new Error("owner group remains");
        ownerGroupAbsent = true;
      });
      if (owner && ownerGroupAbsent) fallbackOwners.delete(owner);
      if (!fallbackRecord?.childGroupAbsent) {
        ...
        await attempt("reported child absence", async () => {
          if (record && !await waitForProcessGroupExit(record.pgid, 30, 50, { cleanupTimeoutMs: 10_000 })) throw new Error("reported child group remains");
          if (fallbackRecord) fallbackRecord.childGroupAbsent = true;
        });
      }
      if (!fallbackRecord || fallbackRecord.childGroupAbsent) {
        let persistenceRemoved = !record;
        await attempt("tagged persistence removal", async () => {
          if (record) {
            if (trackPreReadyActions && !injectedPersistenceRemovalFailure) {
              injectedPersistenceRemovalFailure = true;
              persistenceRemovalFailureId = id;
              throw new Error("injected tagged persistence removal failure");
            }
            await rm(record.persistence, { recursive: true, force: true });
          }
          persistenceRemoved = true;
        });
        if (persistenceRemoved) {
          let persistenceAbsent = !record;
          await attempt("tagged persistence absence", async () => {
            if (record) await absent(record.persistence, `${id} fallback persistence absence`);
            persistenceAbsent = true;
          });
          if (record && persistenceAbsent) fallbackRecords.delete(id);
        }
      }
    };
    ...
    try {
      ...
      for (const [id, entry, extra] of owners) {
        ...
        try {
          ...
          expect(groupIsLive(record.pgid), `${id} reported child group after READY`).toBe(true);
      ...
      for (const [id, entry, extra] of owners) {
        ...
        try {
          ...
          fallbackOwners.add(owner); heldOwners.push({ id: failureId, owner });
          ...
          if (failureId === persistenceRemovalFailureId) await expect(access(record.persistence)).resolves.toBeUndefined();
          else await absent(record.persistence, `${failureId} fallback persistence absence`);
        } finally {
          if (!fallbackCalled) await cleanupFallback(failureId, owner, record, true, true);
        }
      }
      expect(injectedFallbackFailure).toBe(true);
      expect(injectedPersistenceRemovalFailure).toBe(true);
      expect(persistenceRemovalFailureId).toBe("fixture-before-ready");
      ...
      expect(preReadyFallbackActions).not.toContain("fixture-before-ready:tagged persistence absence");
      expect(cleanupErrors.some((error) => String(error).includes("injected fallback owner TERM/KILL failure"))).toBe(true);
      expect(cleanupErrors.some((error) => String(error).includes("injected tagged persistence removal failure"))).toBe(true);
    } finally {
      ...
      for (const [id, fallbackRecord] of fallbackRecords) {
        try {
          if (!fallbackRecord.childGroupAbsent) {
            outerReportedChildSignals.push(id);
            await forceProcessGroupExit(fallbackRecord.record.pgid, `${id} outer fallback reported child`);
          }
          await rm(fallbackRecord.record.persistence, { recursive: true, force: true });
          await absent(fallbackRecord.record.persistence, `${id} outer fallback persistence absence`);
          fallbackRecords.delete(id);
        } catch (error) { cleanupErrors.push(error); }
      }
      expect(outerReportedChildSignals).not.toContain(persistenceRemovalFailureId);
      if (fallbackRecords.size === 0) await rm(controls, { recursive: true, force: true });
      else cleanupErrors.push(new Error(`outer fallback retained ${fallbackRecords.size} reported child group(s)`));
      if (cleanupErrors.some((error) => !String(error).includes("injected fallback owner TERM/KILL failure") && !String(error).includes("injected tagged persistence removal failure"))) throw new AggregateError(cleanupErrors, "owner harness fallback cleanup failed");
    }
  }, 600_000);
  ...
  it("keeps timeout primary over its SIGTERM cleanup and waits for complete group absence", async () => {
    ...
    try {
      ...
      pgid = Number(await readFile(pidFile, "utf8"));
      expect(pgid).toBeGreaterThan(0);
      await expect(waitForProcessGroupExit(pgid, 20, 50)).resolves.toBe(true);
      ...
      try {
        if (!pgid) {
          try { pgid = Number(await readFile(pidFile, "utf8")); }
          catch (error) {
            if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error;
            // Process never published ownership.
          }
        }
        if (pgid && !groupVerifiedAbsent) await forceProcessGroupExit(pgid, "timed-out owner");
      } finally { await rm(resolve(pidFile, ".."), { recursive: true, force: true }); }
    }
  });
  ...
  it("waits for TERM-resistant parent and descendant readiness, then SIGKILL-escalates the whole group", async () => {
    ...
    let groupVerifiedAbsent = false;
    try {
      await Promise.all([waitFor(ready), waitFor(`${ready}.parent`)]);
      await stopOwnedProcess(child, { cleanupTimeoutMs: 1_000 });
      await expect(waitForProcessGroupExit(pgid, 5, 10)).resolves.toBe(true);
      groupVerifiedAbsent = true;
    } finally {
      try { if (!groupVerifiedAbsent) await forceProcessGroupExit(pgid, "TERM-resistant owner"); }
      finally { await rm(resolve(ready, ".."), { recursive: true, force: true }); }
    }
  });
  ...
  it("fails closed when the direct group probe fails", async () => {
    ...
    try {
      ...
      await expect(waitForProcessGroupExit(pgid, 5, 10)).resolves.toBe(true);
      groupVerifiedAbsent = true;
    } finally { if (!groupVerifiedAbsent) await forceProcessGroupExit(pgid, "failed group probe owner"); }
  });
  ...
  it("attempts stop and failing tagged removal, exits descendants, and preserves generated primary failure diagnostics", async () => {
    ...
    const pgid = child.pid!; const primary = new Error("primary failure from owner use path"); let stopAttempts = 0; let removalAttempts = 0; let groupVerifiedAbsent = false;
    try {
      ...
      groupVerifiedAbsent = true;
      expect((primary as Error & { cleanupDiagnostics?: Error }).cleanupDiagnostics?.message).toContain("tagged persistence removal failed");
    } finally {
      try { if (!groupVerifiedAbsent) await forceProcessGroupExit(pgid, "failing tagged removal owner"); }
      finally { await rm(persistence, { recursive: true, force: true }); }
    }
  });
});
