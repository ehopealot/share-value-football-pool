...
describe("My wagers page", () => {
  it("uses the compact Activity-style wager, stake, payout, and P&L layout in each status section", () => {
    ...
    expect(source).toContain('<td><span className="wager-start-time">{starts[index]}</span><span className="wager-start-time-mobile">{mobileStarts[index]}</span></td><td><WagerLine leg={leg}/></td>');
